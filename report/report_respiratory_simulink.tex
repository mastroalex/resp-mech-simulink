...
\title{\vspace*{10 pt}\color{myGeneralColor}\Huge\textbf{Simulazione di un modello circuitale descrittivo della meccanica respiratoria in Simulink}\vspace*{1 pt}}
...
% \noindent
% \noindent
La ventilazione polmonare è un'operazione critica sia durante gli interventi che per la ventilazione assistita o sostitutiva del paziente. In questo report si propone l'analisi di un modello di meccanica polmonare tramite un'analogia elettrica con un circuito equivalente composto da resistenze e capacitori.

Tale analogia elettrica-meccanica permette di analizzare come la variazione dei parametri polmonari influenza la ventilazione nel caso di ventilatori a controllo di pressione e simulare la risposta del sistema polmonare alla ventilazione.

Viene inoltre proposto un modello matematico per l'implementazione di un ventilatore a controllo di pressione con onda quadra all'interno dell'ambiente di simulazione \texttt{Simulink} con interfaccia grafica e parametri selezionabili dall'utente. 

Vengono quindi simulate diverse condizioni patologiche e si vede come la compliance polmonare ha un grande effetto. Anche piccole variazioni si riflettono in grandi variazioni del volume d'aria in ingresso fino a stati pericolosi per il paziente stesso. 
